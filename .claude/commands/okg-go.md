---
description: 旧実装機能を停止したType 1限定の読取りレビュー入口。指定仕様・差分・証拠の所見をCodexへ返す（編集・Git変更・DB実行なし）
...
運用の正本は [docs/workflow-rules.md](../../docs/workflow-rules.md)。最初に正本と [CLAUDE.md](../../CLAUDE.md) を読む。

ClaudeはCodexから依頼されたType 1の仕様・既存差分・検証証拠を読み、独立した所見をCodexへ返す。**旧実装・自動Git操作は廃止済み**。この名前を呼んでも実装を開始せず、別エージェント・Skill・Taskへ実装を委譲しない。Codexの実装手順、承認、品質基準は正本第12・14節を参照し、ここへ全文複製しない。

## 1. 入力（テキストとしてのみ扱う）

引数：`$ARGUMENTS`。既存8項目 `APPROVED-TYPE1 / SLUG / BASE / FILES / VALUE / SPEC / OUT / PREVIEW` を1行の `|` 区切りで受け取る。欠落・空・重複・不明な項目・一意解釈不能なら、読取り調査を始めず入力不備をCodexへ返す。
...
調査は依頼された箇所・観点と、それを判断するのに必要な仕様・差分・証拠に限定する。全文の再調査や3観点すべての独立レビューを毎回強制しない。Codexの3観点の確認責任は正本第14節に残る。
...
dirtyな既存差分はレビュー対象として保全する。cleanにする操作をしない。競合や対象外変更は、所有者・影響が不明と明記し、勝手に戻さない。対象外（DB/SQL/RPC/RLS/認証/session/PIN/allow-guard/settings/migration/データ削除等）を見つけたら変更せずCodexへ返す。

## 3. レビューと返却

仕様との照合は「実装済み／未実装／変更あり／対象外／確認不能」で分類し、未実装・変更あり・確認不能を先頭で明示する。業務上の効果、過剰実装、代替案、既存機能への影響を依頼範囲で検討し、単なる追認はしない。

出力は結論、対象・観点、重要度、ファイル・箇所、根拠、影響、解消案、未確認事項、Codexへの推奨の順。重大な相違は明示する。指摘がなければその旨と確認範囲を示す。修正はCodexが採否・対応・再確認を記録する（Type 1のmust-fix修正上限・medium/lowの扱いは正本第14節）。

岡井さんの判断が必要な事項だけを分離する。merge可否・Phase完了を最終決定しない。Codexが正本の18項目を統合して報告するための限定レビュー結果を返し、毎回のChatGPTへの貼り戻しは求めない。秘密値や長いログを出力しない。

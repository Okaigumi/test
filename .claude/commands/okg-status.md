---
description: 現在の Git / PR / checks / 工程状態をread-onlyで収集し、Codex窓口が取りまとめるための短い状態報告を返す（write操作なし・独立レビューや最終判断はしない）
...
最初に [運用正本](../../docs/workflow-rules.md) 第12・14節と [CLAUDE.md](../../CLAUDE.md) を参照する。結果はCodex窓口へ返し、岡井さんへの重要判断だけを分離する。

新体制では任意のClaude側補助手段として使う。日常の状態取得はCodexが行い、本コマンドと岡井さんによる転送を必須工程にしない。必要な個別依頼があるときだけ、この許可範囲で実行する。
...
**「完了」「merge してよい」等の最終判断は下さない。** 事実と状態所見をCodex窓口へ返し、Codex窓口が検証証拠とともに岡井さんへ必要事項を報告する。この状態報告を、変更と検証証拠に対するClaudeの独立レビューとして扱わない。
...
- 結果をCodex窓口へ返す。Codex窓口が岡井さんへ必要事項を報告し、根拠の照合、範囲内の対応、証拠作成と記録を担当する。岡井さんにエージェント間の伝言を原則求めない。
- 報告は簡潔に。冗長な繰り返しをしない。

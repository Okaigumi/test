> **現行の外部保管手順**：現在の利用方法は [機密資産の外部保管](private-assets-local-usage.md) を参照。DB／Storageバックアップは外部パス指定が必須です。以下の旧パス・旧コマンドは履歴であり、現行手順として実行しないでください。

**履歴と現行手順の区別**：過去の実行・検証記録は現在の実行環境の動作確認を代替しません。端末別の保存先・移設・資産照合結果はGit管理外で管理します。新たな取得には現行の外部パス付き手順を使用し、担当・承認は [運用正本](workflow-rules.md) 第12節に従います。DB実行の本人ゲート、復元対象・禁止事項を維持し、この記載を実行承認にはしません。

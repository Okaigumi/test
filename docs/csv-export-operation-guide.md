...
#### 現時点の実装状況（Phase 2-4-9-1）

実装コミット：`08be5ac Add CSV ZIP report selection menu`

- ZIP読込後は帳票選択メニューが表示される。
- 通常は見たい帳票カードを選ぶ。
- 現時点では、月次チェック・差異確認が実接続済み。
- 工事一覧・原価概要、日報・労務費、請求書費用、重機台帳は次フェーズ（2-4-9-2）で表示対応予定（現状は準備中表示）。
- 月次チェック・差異確認では、確認リスト・差異確認・工事詳細を確認できる。
- 次フェーズでは、ZIP読込後の帳票カードからZIP由来rowsを単体CSVビュー形式で表示する方針（Phase 2-4-9-2-a 設計）。`project_cost_details` 0件は正常な空表示として扱う。
...
## Phase 2-4-9-8-d〜8-f：出力導線のZIP一本化と帳票確認画面との整合

Phase 2-4-9-8-d〜8-f で、管理コンソール側の出力導線を「CSV一式をZIPで出力」に一本化し、帳票確認画面では「管理コンソールで出力したZIP」を選択して自動読込する流れに整理した。ZIP以外の出力導線は通常画面から非表示とした。運用手順としては、管理コンソールで「CSV一式をZIPで出力」→出力されたZIPを帳票確認画面で選択→自動読込→帳票選択メニューから各帳票を確認、という流れになる。

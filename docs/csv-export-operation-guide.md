...
## 16. Phase 2-5-a：運用前棚卸し・試運用チェックリスト

CSV出力・ローカル帳票確認ビューアーを実運用へ移す前に、運用開始前に何を確認すべきかを棚卸しする。本格デバッグではなく、試運用で同じ手順を回すための確認項目・進め方の整理である。

### 16.1 動作確認チェックリスト

試運用では、毎回以下を同じ手順で確認する。

- [ ] 管理コンソールで「CSV一式をZIPで出力」からZIPを出力できる
- [ ] 出力したZIPをローカル帳票確認ビューアーで選択できる
- [ ] ZIPを選択するだけで自動読込される（読込ボタン操作が不要）
- [ ] 帳票選択メニューに対象期間（例：2026年6月分）が表示される
- [ ] 工事一覧・日報・請求書・重機台帳をそれぞれ開ける
- [ ] 月次チェック・差異確認を開ける
- [ ] 工事詳細を開ける
- [ ] 「印刷・PDF保存」が使える
- [ ] 印刷/PDF時に操作ボタン類（戻る・印刷ボタン等）が紙面に出ない
- [ ] 工事名が1文字ずつ縦折れしない
- [ ] raw CSVファイル名やZIPファイル名が通常画面に出ない
- [ ] エラー時・警告時の表示を確認する（必要時のみ確認事項が出る）

### 16.2 試運用の対象者案

- まずは管理者または社内担当者1名で試す。
- 次に、実際に帳票を見る人1〜2名に触ってもらう。
- いきなり全社展開はしない。

### 16.3 試運用期間案

- 1週間程度を目安とする。
- 期間中は毎回同じ手順でCSV一式ZIPを出力・確認する。
- 問題があれば、画面名・操作・発生タイミング・使用したZIP（ファイル名／対象期間）をメモする。

### 16.4 ミス・不具合時の対応案

- ZIPを選び間違えた場合は、正しいZIPを再選択する。
- 表示がおかしい場合は、管理コンソールから再出力する。
- 読込エラーが出た場合は、ZIPを再出力して再試行する。
- それでも直らない場合は、ZIPファイル名・対象期間・画面名を控えて管理者へ連絡する。

### 16.5 運用開始前に未解決として残す項目

- 月次チェック診断文・深いサブページ注記に一部CSV名が残る（通常運用では軽微）。
- 非表示領域に個別CSV出力等の内部向け文言が一部残る（通常画面には出ない）。
- 確認リスト（#multiCrossArea）の印刷可読性は、必要に応じて後フェーズで調整する。
- 請求書PDF登録プロトタイプ（`4c77c20`）は別系統として扱い、本フェーズでは触れない。

## 17. Phase 2-5-b-1：実データZIPでの初回試運用確認
...
- 月次チェック・差異確認を開けた。
- 工事詳細を開けた。
- 印刷・PDF保存ボタン（ZIP由来単体ビュー／月次チェック／工事詳細の3か所）が動作し、window.print() が発火した。
- NaN は出なかった。
- 工事名の縦折れはなかった。
- 通常画面に raw CSV名・ZIP名は出なかった。

### 17.4 確認事項
- project_cost_details.csv は0行CSVだった。
- 2026年6月分は請求書明細0件。
- 請求書費用カードには「この期間の請求書明細は0件です。対象期間に請求書登録がない場合は正常です」と表示され、設計どおりの正常系として確認できた。

### 17.5 残課題
- 月次チェック＞読み込み状況の診断カードにCSV名が残る。
- #multiCrossArea の確認リスト印刷可読性は未最適化。
- 異常系エラー文面に JSZip 由来の技術的英文が出るため、必要に応じて日本語化を検討する。
- 請求書明細ありの期間でも、後日もう1回確認できると望ましい。

### 17.6 判断
実データZIPでの初回通し確認はクリア。機能ブロッカー、NaN、重大な表示崩れは確認されなかったため、Phase 2-5-b の1週間試運用へ進める。

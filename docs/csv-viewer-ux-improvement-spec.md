...
## 30. Phase 2-4-9-7：高齢者向けGUI改善：CSV一式ZIP主導線化＋可読性・画面整理 実装結果
...
実運用では単体CSV読込・個別CSV読込を使わず、CSV一式ZIP読込を通常操作とする。高齢者でも最初に押すべき場所が分かるよう、初期導線をCSV一式ZIP読込に一本化し、不要な選択肢を通常画面から外す。あわせて、文字サイズ・コントラスト・見出し・ボタンの可読性を改善する。

### 30.4 Frontend Design 観点
...
- 初期表示をCSV一式ZIP読込に変更
- currentMode 初期値を multi に変更
- 初期化時に setMode('multi') を呼ぶように変更
- 単体CSVタブを通常画面から非表示
- 単体CSVファイル選択行を通常画面から非表示
- トップ右側の切替ボタン群を通常画面から非表示
- 初期案内文をZIP前提に変更
- ZIP読込カード見出しを「CSV一式ZIPを読み込む（通常はこちら）」に変更
- 「複数CSV統合」という利用者向け表現を「CSV一式ZIP読込」系へ整理
- 個別CSV読込カードを通常画面から非表示
- 「トラブル対応用」「通常は使いません」「通常は開きません」などの案内も通常画面から非表示

### 30.6 帳票選択メニューの改善

- ZIP読込後に帳票選択メニューが目立つように調整
- 帳票選択メニュー見出しを大きく・濃くした
- メニュー冒頭説明文を短縮
- 個別帳票4カードを先に表示
- 横断チェックカードを表示
- 読込結果の細かい情報はメニュー末尾へ移動

### 30.7 横断チェックの色調整

- 横断チェックだけ強い色付きで目立つ状態を解消
- 青背景を撤去
- 通常カードと同じ白背景・通常枠に統一
- 警告・エラーのように見えないように調整
- 月次チェック・差異確認機能は維持
...
- ボタン文字を15px程度へ調整
- 極小文字を12〜13px程度へ底上げ
- muted / faint 系の色を濃くして読みやすく調整
- 補足文字が薄すぎないように調整
- ボタン高さを押しやすい水準へ調整

### 30.9 単体CSV読込・個別CSV読込の扱い

- 単体CSV読込導線は通常画面から非表示
- 個別CSV読込カードも通常画面から非表示
- DOM要素と既存イベント配線は互換性のため残置
- handleText / buildSingleStateAndRender は削除しない
- ZIP由来単体ビューで必要な既存描画処理を維持
- 通常運用ではCSV一式ZIP読込を使う
...
- 実ブラウザでの目視確認
- 実ブラウザでの印刷プレビュー確認
- 実ZIPファイルを使ったブラウザ実読込確認

補足：jsdomでは初期表示、導線非表示、帳票選択メニュー、ZIP由来単体ビュー、月次チェック、工事詳細、window.print() 発火を確認済み。ただし、実ブラウザでの視覚確認、印刷プレビュー、実ZIP読込は運用前に1回実施する。
...
## 32. Phase 2-4-9-8-b：ZIP読込後に帳票選択メニューをさらに上位へ表示

- **実装コミット**：6ec1ea3 Prioritize report menu after ZIP load
- **実装目的**：ZIP読込後、ZIP読込カードのパッケージ詳細・ZIP内ファイル一覧が画面上部を占め、肝心の帳票選択メニューが下に押し下げられていた（§31.9 残課題）。高齢者利用者が「読み込んだ次に何をすればよいか」を迷わないよう、帳票選択メニューを最優先で目に入る位置へ引き上げる。
- **実装内容**：
  - `#multiPackageArea`（読み込んだCSV一式の詳細）を帳票選択メニューの後ろへ移設。
  - ZIP読込カードは読込後もコンパクト表示にとどめる。
  - パッケージ詳細・ZIP内ファイル一覧・manifest・警告情報を `<details>/<summary>` の折りたたみ表示へ変更。
  - 通常は折りたたみ、警告・エラーがある場合のみ自動展開。
- **実ブラウザ確認結果**：menu top=383px、package details top=1392px を確認し、帳票選択メニューが読込詳細より明確に上位へ出ていることを確認。Console重大エラーなし。
- **変更しなかったもの**：CSV解析・ZIP読込本体・集計・月次チェック内部処理は無変更。window.print() と @media print は無変更。DB / SQL なし。
- **残課題**：月次チェック「簡易工事一覧」横長表の印刷最適化（別フェーズ）。

## 33. Phase 2-4-9-8-d：帳票確認画面の文言・自動読込・配色整理

- **実装コミット**：512f72e Refine ZIP-first report viewer and admin export flow
- **実装目的**：「CSVビューア」「CSV確認ビューアー」等の技術的呼称や raw CSV名・出力日時などの内部情報を高齢者利用者の通常画面から外し、「管理コンソールで出力したZIPを選択して帳票を確認する」流れを一目で理解できるようにする。
- **実装内容**：
  - 画面名（タイトル・パネル見出し・帳票ヘッダ・単体ビュー見出し・印刷sub）を「帳票確認」へ整理。
  - TOP説明文を「管理コンソールで出力したZIPを選択すると、工事一覧・日報・請求書・重機台帳を確認できます。」へ変更。
  - ZIP読込カード見出しを「管理コンソールで出力したZIPを選択」、補足を「ZIPを選択すると自動で読み込みます。」へ変更。
  - ZIP選択後に自動読込（読込ボタン操作を不要化）。
  - ZIP読込ボタンを通常画面から非表示。
  - 帳票選択メニュー見出しに対象期間（例：2026年6月分）を表示。
  - raw CSVファイル名・ZIPファイル名・出力日時・ZIP内ファイル一覧を通常画面から非表示。
  - 正常時の「読み込んだCSV一式の情報」を通常画面から撤去し、警告・エラー時のみ確認事項を表示。
  - フォントを BIZ UDPGothic / Yu Gothic UI / Meiryo 系へ調整。
  - 薄いブルー系＋白の配色へ調整。
- **実ブラウザ確認結果**：title・各見出しが「帳票確認」、TOP説明文・ZIP読込カード文言が新文言、ZIP選択での自動読込（「読み込みました。帳票を選んでください。」）、帳票選択メニュー見出しに対象期間表示、ZIP読込ボタン・mode-tabs が display:none を確認。Console重大エラーなし。
- **変更しなかったもの**：CSV解析・ZIP読込本体・集計・月次チェック内部処理は無変更。window.print() と @media print は無変更。DB / SQL なし。
- **残課題**：`#preLoad` の非表示文言・非表示領域内の内部向け文言が一部「読み込む」「個別CSV」表記のまま（通常画面非表示のため軽微）。

## 34. Phase 2-4-9-8-e：工事名縦折れ改善

- **実装コミット**：512f72e Refine ZIP-first report viewer and admin export flow
- **実装目的**：工事一覧・原価概要や月次チェックの簡易工事一覧で、列幅が狭まると日本語の工事名が1文字ずつ縦に折り返され、極端に読みにくくなる問題を解消する。横スクロールや文字縮小は高齢者可読性を損なうため採らない。
- **実装内容**：
  - 工事名を表セル内ではなく上段見出し（フル幅タイトル）として分離。
  - 工事一覧・原価概要と月次チェック内の簡易工事一覧をカード/グリッド型に整理。
  - 工事名リンクは維持。
  - 各工事の属性・数値はラベル付きグリッド（`repeat(auto-fit, minmax(...))`）で表示し、横スクロール・font縮小なしで折り返さない。
- **実ブラウザ確認結果**：長い工事名（例「東町道路改良工事」）がフル幅・1行で表示され、1文字ずつの縦折れが発生しないことを確認。工事名リンクは有効。
- **変更しなかったもの**：CSV解析・集計ロジックは無変更。window.print() と @media print は無変更。DB / SQL なし。
- **残課題**：月次チェック横長表そのものの印刷最適化は別フェーズ。

## 35. Phase 2-4-9-8-f：管理コンソールCSV出力導線のZIP一本化

- **実装コミット**：512f72e Refine ZIP-first report viewer and admin export flow
- **実装目的**：管理コンソール側のCSV出力に「CSV一式ZIP」と「個別CSV」が併存していたため、出力者が迷う。帳票確認画面が「管理コンソールで出力したZIP」を前提にした以上、出力側もZIP一本に揃え、出力→選択→確認の流れの文言を統一する。
- **実装内容**：
  - CSV出力を「CSV一式をZIPで出力」に一本化。
  - 「CSV一式をZIPで出力（推奨）」から「（推奨）」を削除（推奨の対比相手がなくなったため）。
  - ZIP以外の個別CSV出力導線を通常画面から非表示（内部関数・DOMは互換のため残置）。
  - ZIP出力失敗時の案内から「個別CSV出力をご利用ください」を削除し、再試行・管理者連絡の案内へ変更。
  - 管理コンソール「CSV一式をZIPで出力」→帳票確認「管理コンソールで出力したZIPを選択」の流れに文言を統一。
- **実ブラウザ確認結果**：CSV出力UIに「📦 CSV一式をZIPで出力」「⬇ CSV一式をZIPで出力」を表示、「推奨」不在、個別CSV出力カード群が `<div hidden>`（display:none）で非表示、配信ファイルに「個別CSV出力をご利用ください」が無いことを確認。管理コンソールはPINログイン画面まで（実データ操作は回避）。
- **変更しなかったもの**：ZIP出力処理本体・CSV生成ロジック・CSV列仕様・DB処理・Supabase処理は無変更。window.print() と @media print は無変更。DB / SQL なし。
- **残課題**：非表示領域（`<div hidden>`）内の補足文に「個別CSV出力」表記が残る（画面非表示のため軽微）。

## 36. Phase 2-4-9-8-c：月次チェック横長表の印刷最適化

### 36.1 実装コミット
fe80fbc Optimize monthly check print layout

### 36.2 実装目的
月次チェック・差異確認の印刷/PDF保存時に、簡易工事一覧の横長情報が紙面で見切れたり、工事名が縦折れしたりする問題を改善する。印刷を画面の写しではなく、社内確認に使える紙面として整える。

### 36.3 実装内容
- @media print 内に project-summary 系カード/ブロック用の印刷CSSを追加
- .project-list / .project-summary-block / .project-name-title / .project-summary-grid / .project-summary-item / .psi-label / .psi-val の印刷時表示を調整
- 工事ごとのブロックに break-inside: avoid / page-break-inside: avoid を設定
- 工事名を上段見出しとして印刷
- 数値・属性をラベル付き4列グリッドで印刷
- 罫線を印刷時に黒系へ調整
- 右端列が見切れない構成へ調整
- 操作ボタン類は既存の印刷非表示ルールで出さない
...
- 月次チェック・差異確認を開ける
- 簡易工事一覧の画面表示に悪影響なし
- 印刷メディアで工事ごとの罫線ブロック表示を確認
- 工事名の1文字縦折れなし
- 数値・属性がラベル付き4列グリッドで読める
- 右端見切れなし
- PDF生成成功
- window.print() 発火に影響なし
- Console重大エラーなし
...
### 36.7 残課題
- 確認リスト（#multiCrossArea）の5列テーブルは既存の印刷ルールのまま
- 月次チェック見出しの対象期間表示とmanifest形式の対応は別件
- #preLoad 非表示文言、非表示領域の個別CSV表記、診断文のCSV名は既知残課題

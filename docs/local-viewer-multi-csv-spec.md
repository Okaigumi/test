...
### project_cost_details ZIP由来単体ビュー接続 実装済み（Phase 2-4-9-2-e）

実装コミット：`34d5d73 Connect ZIP project cost details to single CSV viewer`。詳細は [`docs/csv-viewer-ux-improvement-spec.md`](csv-viewer-ux-improvement-spec.md) §18。

- 「請求書費用」カードから ZIP内 `project_cost_details.csv` を単体CSVビュー形式で表示できるようにした。0件CSVでも正常表示する。
- `projects_summary` / `attendance_details` / `project_cost_details` は接続済み。`machine_details` は引き続き準備中表示。

### machine_details ZIP由来単体ビュー接続 実装済み（Phase 2-4-9-2-f）

実装コミット：`26d7a30 Connect ZIP machine details to single CSV viewer`。詳細は [`docs/csv-viewer-ux-improvement-spec.md`](csv-viewer-ux-improvement-spec.md) §19。
...
### ZIP読込後UXの微調整 設計（Phase 2-4-9-3）

設計の詳細は [`docs/csv-viewer-ux-improvement-spec.md`](csv-viewer-ux-improvement-spec.md) §21。

- 帳票選択メニューを起点に、4帳票の個別確認と月次チェック・差異確認の横断確認を分けて整理する。
...
### 高齢者向けGUI改善：CSV一式ZIP主導線化＋可読性・画面整理 実装済み（Phase 2-4-9-7）

実装結果の詳細は [`docs/csv-viewer-ux-improvement-spec.md`](csv-viewer-ux-improvement-spec.md) §30（コミット `b15bb7c` / `31c4986`）。

- CSV一式ZIP読込を通常導線として初期表示するようにした。単体CSV読込導線と個別CSV読込カードは通常画面から外し、ZIP読込後は帳票選択メニューを主役として表示するように整理した。横断チェックカードは通常カードと同じ白背景・通常枠に統一し、高齢者向けに文字サイズ・見出し・説明文・コントラスト・ボタン可読性も改善した。
...
## Phase 2-4-9-8-b〜8-f：CSV一式ZIP読込後の表示・文言・出力導線整理

Phase 2-4-9-8-b で、CSV一式ZIP読込後の表示順を見直し、帳票選択メニューと個別帳票4カードが先に表示されるようにした。読み込んだCSV一式の詳細、ZIP内ファイル一覧、manifest、警告情報は折りたたみ表示へ移動し、警告・エラーがある場合のみ自動展開する（実装コミット 6ec1ea3）。

Phase 2-4-9-8-d で、CSV一式ZIP読込の利用者向け文言を「管理コンソールで出力したZIP」へ整理し、ZIP選択後の自動読込に変更した。帳票選択メニュー見出しには対象期間を表示し、正常時の詳細な読込情報（raw CSVファイル名・ZIPファイル名・出力日時・ZIP内ファイル一覧）は通常画面から非表示にした。あわせて画面名を「帳票確認」へ整理し、フォント・配色を調整した（実装コミット 512f72e）。

Phase 2-4-9-8-e で、工事一覧・原価概要および月次チェック内の簡易工事一覧をカード/グリッド型へ変更し、工事名の縦折れを解消した。横スクロール・文字縮小は用いず、工事名を上段見出しとして分離した（実装コミット 512f72e）。

...
>
> **Phase 2-4-9-5（設計）**：帳票確認後に保存・共有しやすくするため、印刷・PDF保存導線を整備する。PDFライブラリは追加せず、ブラウザ標準の `window.print()` を使う方針とする。設計は [`docs/csv-viewer-ux-improvement-spec.md`](csv-viewer-ux-improvement-spec.md) §25 を参照。
>
...
>
> **Phase 2-4-9-6（確認完了）**：CSVビューアーの運用前最終確認を実施した。ZIP読込、4帳票単体ビュー、月次チェック・差異確認、工事詳細表示、個別CSV読込、印刷CSS、セキュリティ確認、JS構文チェックを確認し、運用開始可能と判定した。確認結果は [`docs/csv-viewer-ux-improvement-spec.md`](csv-viewer-ux-improvement-spec.md) §29 を参照。
...
## Phase 2-4-9-8-b〜8-f：帳票確認画面の表示順・文言・配色・出力導線整理

Phase 2-4-9-8-b で、ZIP読込後に帳票選択メニューがより早く目に入るように表示順を調整した。パッケージ詳細・ZIP内ファイル一覧・manifest・警告情報は帳票選択メニューの後ろの折りたたみ詳細へ移動し、通常時は画面上部を占有しないようにした（実装コミット 6ec1ea3）。

Phase 2-4-9-8-d で、帳票確認画面の文言を「管理コンソールで出力したZIPを選択して帳票を確認する」流れに整理した。ZIP選択後の自動読込、帳票選択メニューの対象期間表示、raw CSVファイル名・ZIPファイル名・出力日時の通常画面非表示、フォント調整、薄いブルー系＋白の配色を実施した（実装コミット 512f72e）。

Phase 2-4-9-8-e で、工事名が1文字ずつ縦折れする問題を解消した。横スクロールや文字縮小ではなく、工事名を上段見出しとして分離し、各工事の属性・数値をラベル付きグリッドで表示する構成へ変更した（実装コミット 512f72e）。

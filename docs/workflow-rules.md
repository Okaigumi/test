...
作成日：2026-06-13
対象：社内業務システムプロジェクト全体
更新日：2026-09-11（判断・承認と操作の実行担当を明確化）
...
## 12. Codex主体の役割・承認・実行手順（2026-09-10承認、2026-09-11実行委任を明確化）

### 役割

- 岡井さんは業務要件、優先順位、使い勝手と、仕様・変更範囲・公開・mergeの判断・承認を担当する。本人による目視・認証が必要な確認は、その必要箇所に限定する。
- Codexは主担当として、承認範囲内の調査、仕様案、実装、検証、自己修正、記録、Git操作、PR操作、merge、デプロイ結果確認を実行する。判断根拠と未確認事項を示し、必要な独立レビューの対象を絞る。岡井さんがmergeを承認することと、岡井さん自身がmerge操作を行うことは区別する。
...
- 対象PR、変更範囲、merge方式、付随する自動公開まで明示承認されていれば、Codexは承認されたGit／PR操作からmerge・デプロイ結果確認まで一貫して実行する。同じ承認を工程ごとに再要求せず、本人によるボタン操作も一律には要求しない。
- 承認は対象限定であり、将来の別PR、追加変更、DB操作、設定変更への包括許可ではない。Claudeの編集・Git操作権限はこの実行委任では広げず、原則として限定的な独立レビュー担当とする。
...
5. 第7節の記録先へ結果・証拠・未確認事項を残し、第9・10節に沿って報告する。実行結果と証拠で確認できた範囲を区別し、Previewの証拠を本番確認済みとして流用しない。非公開の実環境情報・内部証拠本文はGit管理外へ保全する。未承認のGit操作、DB、公開等へ進まない。
...
- 承認済み仕様を入力とし、**preflight → branch → 調査 → Type1/価値確認 → 実装 → 3レビュー（frontend-design・security・test-evidence 必須）→ commit → push → PR作成 → checks → merge判断報告** を品質上の工程とする。Codexが実装と3観点の自己点検を担当し、Claudeの独立レビューは第12節の必要箇所に限定する。Git／PR操作はその操作の承認がある場合だけ実行する。merge・付随する公開の承認がなければmerge判断報告で停止し、承認済みなら下記条件に従ってCodexが続行する。`/okg-go`自体は実装・Git操作を行わず、必要な独立レビューのみを返す。
...
- **mergeの判断・承認者は岡井さん、承認された操作の実行担当はCodexとする。** Claudeはmergeコマンドを提示も実行もしない。Codexは証拠に基づく推奨を示し、対象PR・変更範囲・merge方式・付随する自動公開の明示承認を確認して実行する。岡井さん自身のボタン操作は必須条件にしない。
- 流れ: Codexがmerge判断報告 → 必要箇所の独立レビューと指摘解消 → 岡井さんによる対象限定の承認（既存承認があれば再要求しない）→ Codexが検証済みHEADとPRの対象HEAD一致・checks成功・保護条件を直前確認 → 承認された方式でmerge → mainへの反映・自動デプロイの対象SHAと結果を確認・記録する。
